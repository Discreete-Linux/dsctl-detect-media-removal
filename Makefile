...
PYMODS = 
BINFILES =
USRBINFILES = removal-warning.py
EXTENSIONS = 
EXTRATARGETS = udev-watchdog
EXTRAINSTALLS = memlockd
###
...
USRSBINFILES = udev-watchdog udev-watchdog-wrapper
MEMLOCKCFG = dsctl-detect-media-removal.cfg
MEMLOCKCFGDIR = $(DESTDIR)/etc/memlockd.d
###
# Standard-Rezepte
...
clean:
	rm -rf *.pyc udev-watchdog

distclean:
	rm -rf *.pyc *.gz udev-watchdog
...
udev-watchdog:	udev-watchdog.c
	gcc -I/usr/include/$(ARCHINCLUDE) -o udev-watchdog udev-watchdog.c -Wall -ludev -lrt && \
	strip --strip-all udev-watchdog
	
memlockd:
ifneq ($(MEMLOCKCFG),)
	mkdir -p $(MEMLOCKCFGDIR)
	install -m 644 $(MEMLOCKCFG) $(MEMLOCKCFGDIR)
endif
